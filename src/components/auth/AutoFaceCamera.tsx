...
export function AutoFaceCamera({onScan,paused,pausedLabel='Face matched'}:{onScan:(image:Blob)=>Promise<void>;paused:boolean;pausedLabel?:string}){
  ...
  useEffect(()=>{
    ...
    const scan=()=>{
      ...
      if(pausedRef.current||scanningRef.current||!video||video.readyState<2)return
      const width=Math.min(video.videoWidth,480);const height=Math.round(width*(video.videoHeight/video.videoWidth))
      const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;canvas.getContext('2d')?.drawImage(video,0,0,width,height)
      scanningRef.current=true
      canvas.toBlob(blob=>{if(!blob){scanningRef.current=false;return}void onScan(blob).finally(()=>{scanningRef.current=false})},'image/jpeg',.75)
    }
    const video=videoRef.current
    const scanWhenReady=()=>{window.setTimeout(scan,250)}
    video?.addEventListener('loadeddata',scanWhenReady)
    ...
    return()=>{mounted=false;window.clearInterval(timer);video?.removeEventListener('loadeddata',scanWhenReady);stream?.getTracks().forEach(track=>track.stop())}
  },[onScan])
