...
export function OrdersPage(){
  ...
  return <div className="p-4 pt-16 lg:p-8">
    ...
    <div className="mb-4 flex flex-wrap items-end gap-3 rounded-2xl bg-white p-4 shadow-sm">
      <div className="flex items-center gap-2 self-center text-sm font-semibold text-stone-700"><CalendarDays size={18} className="text-stone-500"/>Filter date</div>
      <label className="text-xs font-semibold uppercase tracking-wide text-stone-500">Date<input aria-label="Order date" type="date" value={filterDate} onChange={event=>setFilterDate(event.target.value)} className="mt-1 block rounded-xl border px-3 py-2 text-sm font-normal normal-case tracking-normal text-stone-900 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"/></label>
      {hasDateFilter&&<button type="button" onClick={()=>setFilterDate('')} className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-sm font-semibold text-stone-600 hover:bg-stone-100"><X size={16}/>Clear</button>}
    ...
    <div className="overflow-x-auto rounded-2xl bg-white"><table className="w-full min-w-[640px] text-left">
      ...
      </tr>})}{filteredOrders.length===0&&<tr><td colSpan={6} className="p-10 text-center text-stone-500">{hasDateFilter?'No orders found for the selected date.':'No completed orders.'}</td></tr>}</tbody>
    </table></div>
    ...
    {selectedOrder&&<div className="fixed inset-0 z-40 grid place-items-center bg-black/55 p-4" role="dialog" aria-modal="true" aria-labelledby="order-detail-title" onMouseDown={e=>{if(e.target===e.currentTarget)closeOrder()}}><section className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
      <div><h2 id="order-detail-title" className="text-xl font-bold">Order {selectedOrder.order_number}</h2><p className="text-sm text-stone-500">Sold by {selectedOrder.cashier?.full_name??'Unknown staff'} · {new Date(selectedOrder.created_at).toLocaleString()}</p></div>
      ...
      {isOwner&&<><div className="mt-6 border-t"></div><div className="mt-6 flex justify-end"><button onClick={()=>requestDelete(selectedOrder)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-200 px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50"><Trash2 size={16}/>Delete</button></div></>}
    </section></div>}
