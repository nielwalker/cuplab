...
export async function getOrder(id:string){const {data,error}=await supabase.from('orders').select('*, cashier:profiles!orders_cashier_id_fkey(full_name), order_items(*)').eq('id',id).single();if(error)throw error;return data as OrderWithItems}
...
export async function listOrders(){return cachedQuery('order-history',async()=>{const {data,error}=await supabase.from('orders').select('*, cashier:profiles!orders_cashier_id_fkey(full_name)').eq('status','COMPLETED').order('created_at',{ascending:false});if(error)throw error;return data as Order[]})}
