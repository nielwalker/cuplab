...
export async function registerStaff(fullName:string,username:string,password:string){return staffRequest({action:'create',full_name:fullName,username,password})}
...
export async function updateStaff(id:string,fullName:string,username:string,password?:string){return staffRequest({action:'update',id,full_name:fullName,username,...(password?{password}:{})})}
