...
Deno.serve(async req=>{
  ...
  if(input.action==="create"){
    ...
    return json({id:data.user.id,full_name:fullName,username})
  }
  ...
  if(input.action==="update"){
    ...
    const {error:profileError}=await admin.from("profiles").update({full_name:fullName,username}).eq("id",input.id)
    if(profileError)return fail("Staff profile update failed",409)
    return json({id:input.id,full_name:fullName,username})
  }
